...
class VarBinHelper:
    ...
    def bin_cont_init(self, sr_feature, y, bin_rate=0.01, **kwargs):
        ...
        missing = kwargs.get("missing", "unique")
    ...
    def calc_chi2_cat(self, df_bin_interval):
        total_count = df_bin_interval.total.sum()  ## find the total bad count and good count
    ...
    def bin_equal_freq(self, sr_feature, **kwargs):
        ...
        if missing == "unique":  ## if choose unique bin for missing value, add np.nan as bin 0
            ls_bin_up = [np.nan]
    ...
    def map_bin(self, sr_feature, df_bin_interval, **kwargs):
        ...
        missing = kwargs.get("missing", "unique")  ## missing value handling -- by default NAs go to bin 0, a unique bin
        cat = kwargs.get('cat', False)

        var_name = sr_feature.name
        df = sr_feature.copy()  ## the working df, will be returned
        df['bin'] = 0
        idx_offset = 0
        if missing == "unique":
    ...
    def calc_chi2(self, df_mapped, y, df_bin_interval, **kwargs):
        ## expect X should have 2 columns, just the X var + mapping output
    ...
    def merge_pair(self, df_chi2, idx_left, idx_right):  ## merging row with idx_left and idx_right
        ...
        if idx_left > 0:
            row.chi2_after_merge_with_left = row.chi2 + df.loc[idx_left - 1, 'chi2']
        if idx_left + 2 < len(df_chi2):
            ## because the second last row does not have index+2 row
    ...
    def chi2_merge(self, df_chi2, **kwargs):
        ...
        while len(df_chi2) > min_bin:
            sr_chi2 = df_chi2['chi2_after_merge_with_left'][
                      1:]  ## index 0 is NA, 1 is chi2 if bin 1 merge with bin 0, by default we use index 2 onwards
        ...
        if max_bin is not None:  ## further merge bins if there is a max number of bins
        ...
        df_bin_interval.columns = df_bin_interval.columns.tolist()[:-3] + ["total", 'bad', 'bad_rate']
    ...
    def fit(self, X, y, **kwargs):
        ls_cat_feature = kwargs.get("ls_cat_feature", [])
        label = kwargs.get("label", self.label)
        df_bin_model = pd.DataFrame(columns = ['feature','bin'])
        ls_bin = list()
        ls_var = list()
        for var_name in ls_cat_feature:
            print(var_name)
            df_bin_interval = self.fit_single_cat(X[var_name].astype(str), y, **kwargs)
